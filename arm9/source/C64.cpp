...
#else
	i += fread((void*)&state, 18, 1, f); //sizeof(state) = 18 on pc
#endif
...
bool C64::Load1541State(FILE *f)
{
	...
	i += fread((void*)&state, sizeof(state), 1, f);
#else
	i += fread((void*)&state, 50, 1, f); //sizeof(state) is 50 on pc
#endif
#ifdef __NDS__
	if (i == 2) {
#else
...
bool C64::SaveVICState(FILE *f)
{
	...
	return fwrite((void*)&state, 120, 1, f) == 1;
}
...
#else
		int k=fread((void*)&state, 120, 1, f);
#ifdef __NDS__
		if (k == 1) {
#else
	if (k == 120) { //sizeof(state) is 120 on pc
#endif
#endif
...
bool C64::SaveSIDState(FILE *f)
{
	...
	return fwrite((void*)&state, 29, 1, f) == 1;
}
...
bool C64::LoadSIDState(FILE *f)
{
	...
	if (fread((void*)&state, sizeof(state), 1, f) == 1) {
#else
		int k=fread((void*)&state, 29, 1, f);
#ifdef __NDS__
	if (k==1) {
#else
	if (k==29) { //sizeof(state) is 29 on pc
#endif
#endif
...
bool C64::SaveCIAState(FILE *f)
{
	...
	if (fwrite((void*)&state, 26, 1, f) == 1) {
		TheCIA2->GetState(&state);
		return fwrite((void*)&state, 26, 1, f) == 1;
	} else
		return false;
}
...
bool C64::LoadCIAState(FILE *f)
{
	...
	if (fread((void*)&state, sizeof(state), 1, f) == 1) {
#else
		int k=fread((void*)&state, 26, 1, f);
#ifdef __NDS__
	if (k==1) {
#else
	if (k == 26) { //sizeof(state) is 26 on pc
#endif
#endif
		...
		if (fread((void*)&state, sizeof(state), 1, f) == 1) {
#else
		int j=fread((void*)&state, 26, 1, f);
#ifdef __NDS__
	if (j==1) {
#else
	if (j == 26) { //sizeof(state) is 26 on pc
#endif
#endif
			TheCIA2->SetState(&state);
			return true;
		} else
	{ iprintf("LoadCIAState1\n");
	return false;}
	} else
	{ iprintf("LoadCIAState2\n");
	return false;}
}
...
bool C64::Save1541JobState(FILE *f)
{
	...
	return fwrite((void*)&state, 12, 1, f) == 1;
}
...
bool C64::Load1541JobState(FILE *f)
{
	...
	if (fread((void*)&state, sizeof(state), 1, f) == 1) {
#else
		int k=fread((void*)&state, 12, 1, f);
#ifdef __NDS__
	if (k==1) {
#else
	if (k == 12) { //sizeof(state) is 12 on pc
#endif
#endif
		TheJob1541->SetState(&state);
		return true;
	} else
	{ iprintf("Load1541JobState\n");
	return false;}
}
