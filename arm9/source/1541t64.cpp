...
static bool parse_t64_file(FILE *f, c64_dir_entry **dir_entries, char *dir_title);
static bool parse_lynx_file(FILE *f, c64_dir_entry **dir_entries, char *dir_title);
static bool parse_p00_file(FILE *f, c64_dir_entry **dir_entries, char *dir_title);
...
ArchDrive::ArchDrive(IEC *iec, const char *filepath) : Drive(iec), the_file(NULL)
{
	...
	file_info=(c64_dir_entry **)malloc(255*sizeof(c64_dir_entry *));
	file_info[0]=NULL;
...
uint8 ArchDrive::open_file(int channel, const uint8 *name, int name_len)
{
	...
	if (find_first_file(plain_name, plain_name_len, num)) {
		...
		ram_file[channel]=
			(uint8 *)malloc(((*file_info[num]).size)+2);
		ram_file_size[channel]=((*file_info[num]).size)+2;
		ram_file_pos[channel]=0;
			...
			if (archive_type == TYPE_T64) {
				...
				ram_file[channel][0]=(*file_info[num]).sa_lo;
				ram_file[channel][1]=(*file_info[num]).sa_hi;
			...
			memcpy(ram_file[channel]+2, buf, (*file_info[num]).size);
			//rewind(file[channel]);
			delete[] buf;

			if (mode == FMODE_READ)	// Read and buffer first byte
				read_char[channel] = //getc(file[channel]);
			ram_file[channel][ram_file_pos[channel]++];
...
bool ArchDrive::find_first_file(const uint8 *pattern, int pattern_len, int &num)
{
	...
	int i;
	for(i=0, num=0; file_info[i]!=NULL; i++, num++) {
...
uint8 ArchDrive::open_directory(int channel, const uint8 *pattern, int pattern_len)
{
	...
	//	return ST_OK;
	//}
	ram_file[channel]=(uint8 *)malloc(2048);
	ram_file_pos[channel]=0;
	...
	memcpy(&(ram_file[channel][0]), buf, 32);
	ram_file_pos[channel]+=32;
	...
	for(i=0; file_info[i]!=NULL; i++) {
		...
		if (pattern_len == 0 || match(pattern, pattern_len, (*file_info[i]).name)) {
			...
			*p++ = 0x01;
			...
			int n = ((*file_info[i]).size + 254) / 254;
			*p++ = n & 0xff;
			*p++ = (n >> 8) & 0xff;
			...
			*p++ = '\"';
			uint8 *q = p;
			for (int j=0; j<16 && (*file_info[i]).name[j]; j++)
				*q++ = (*file_info[i]).name[j];
			*q++ = '\"';
			p += 18;
			...
			// Write line
			//fwrite(buf, 1, 32, file[channel]);
			memcpy(&(ram_file[channel][ram_file_pos[channel]]), buf, 32);
			ram_file_pos[channel]+=32;
		}
	}
	...
	memcpy(&(ram_file[channel][ram_file_pos[channel]]), 
			"\001\001\0\0BLOCKS FREE.             \0\0", 32);
	ram_file_pos[channel]+=32;

	// Rewind file for reading and read first byte
	//rewind(file[channel]);
	ram_file_size[channel]=ram_file_pos[channel];
	ram_file_pos[channel]=0;
	//read_char[channel] = getc(file[channel]);
	read_char[channel]=ram_file[channel][ram_file_pos[channel]];
	ram_file_pos[channel]++;
...
uint8 ArchDrive::Close(int channel)
{
	...
	if (ram_file[channel]) {
		free(ram_file[channel]);
		ram_file[channel] = NULL;
	}
...
uint8 ArchDrive::Read(int channel, uint8 &byte)
{
	...
	if (!ram_file[channel]) return ST_READ_TIMEOUT;

	// Get char from buffer and read next
	byte = read_char[channel];
	//int c = getc(file[channel]);
	c=ram_file[channel][ram_file_pos[channel]++];
	//if (c == EOF)
	if(ram_file_pos[channel]>=ram_file_size[channel]) {
		return ST_EOF;
	} else {
		//read_char[channel] = c;
		read_char[channel] = c;
...
static bool is_t64_header(const uint8 *header)
{
	return memcmp((const char *)header, "C64S tape file", 14) == 0;
}

static bool is_lynx_header(const uint8 *header)
{
	return memcmp((const char *)header + 0x38, "USE LYNX", 8) == 0;
}

static bool is_p00_header(const uint8 *header)
{
	return memcmp((const char *)header, "C64File", 7) == 0;
}
...
static bool parse_t64_file(FILE *f, c64_dir_entry **dir_entries, char *dir_title)
{
	...
	for (int i=0; i<max; i++, b+=32) {
		if (b[0] == 1) {
			...
			int j;
			for(j=0; dir_entries[j]!=NULL; j++);
			dir_entries[j]=(c64_dir_entry *)malloc(sizeof(c64_dir_entry));
			*(dir_entries[j])=c64_dir_entry(name_buf, FTYPE_PRG, false, false, size, offset, b[2], b[3]);
			dir_entries[j+1]=NULL;
		}
	}
...
static bool parse_lynx_file(FILE *f, c64_dir_entry **dir_entries, char *dir_title)
{
	...
	for (int i=0; i<num_files; i++) {
		...
		for(j=0; dir_entries[j]!=NULL; j++);
		dir_entries[j]=(c64_dir_entry *)malloc(sizeof(c64_dir_entry));
		*(dir_entries[j])=c64_dir_entry(name_buf, type, false, false, size, cur_offset, sa_lo, sa_hi);
		dir_entries[j+1]=NULL;
...
static bool parse_p00_file(FILE *f, c64_dir_entry **dir_entries, char *dir_title)
{
	...
	*(dir_entries[0])=c64_dir_entry(name_buf, FTYPE_PRG, false, false, size, 26, sa_lo, sa_hi);
	dir_entries[1]=NULL;
	return true;
}

bool ReadArchDirectory(const char *path, c64_dir_entry **dir_entries)
{
	// Open file
	...
	if (f) {
		...
		char dir_title[16];
		if (is_t64_header(header))
			result = parse_t64_file(f, dir_entries, dir_title);
		else if (is_lynx_header(header))
			result = parse_lynx_file(f, dir_entries, dir_title);
		else if (is_p00_header(header))
			result = parse_p00_file(f, dir_entries, dir_title);
